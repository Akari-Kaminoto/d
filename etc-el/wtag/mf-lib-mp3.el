...
(defmacro mf-decode-point-codez-string (code term)
...
(if (fboundp 'mf-buffer-substring)
    nil
    ...
    (dolist (a tags (reverse result))
      (cond
       ...
       ((and (null no-binary) (member (car a) '("APIC"))) ;; for MP3
        ...
        ;; 
        ;;  FLAG 2バイトの無視して構わない. CODE はエンコードタイプのバイト長整数.
        ...
        (let* ((code (cdr (assoc (char-after) mf-oma-encode)))
               ...
               (file (mf-decode-point-codez-string code term))
